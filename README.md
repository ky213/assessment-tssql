...
After we calculate remaning days cost (see plans mudule) we can say that this cost is also the cost of the current team (or defaultUsers) for the remaining days, this means that we know the price per user for the remaining days.

So,

newCostPriceUser = remainingDaysCost / current-users

hence: for every added user we add newPricePerUser to the total.
